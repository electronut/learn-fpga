 ...
 * blerp.h
 *
 *	Bilinear interpolation code based on:
...
static inline uint32_t getpixel(image_t *image, unsigned int x, unsigned int y){
...
static void putpixel(image_t *image, unsigned int x, unsigned int y, uint32_t color){
    image->pixels[(y*image->w) + x] = color;
}

static void scale(image_t *src, image_t *dst, float scalex, float scaley){
